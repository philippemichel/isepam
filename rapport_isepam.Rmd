...
tt$sarcopenie <- as.factor(ifelse((tt$sexe == "H" & tt$surface.1.taille < 524)|(tt$sexe == "F" & tt$surface.1.taille < 385),"sarcopénique", "non sarcopénique"))
# imc
...
```{r tabdemo}
numv <- c(1:15,30,31)
tabcph(tt[,numv], sarcopenie, nomv = nomv[numv], titre = "Données démographiques", export = expx)
```
...
```{r}
npat <- nrow(tt)
nsarco <- table(tt$sarcopenie)[[2]]
psarco <- round(100*nsarco/npat,1)
bsarco <- paste0(nsarco," (",psarco,"%)")
```


Le critère principal est l'étude de la durée de séjour en fonction de la sarcopénie définie selon les normes de l'HAS. L'échantillon comporte `r bsarco` patients sarcopéniques.

```{r princi1}
v1 <- var(subset(tt$duree.de.sejour.jour, tt$sarcopenie == "non sarcopénique"))
v2 <- var(subset(tt$duree.de.sejour.jour, tt$sarcopenie == "sarcopénique"))
zz <- wilcox.test(tt$duree.de.sejour.jour~tt$sarcopenie)
...
La différence entre les deux groupes n'est pas significative (`r bp1`). Il est probable que la petite taille de l'échantillon soit en cause.

```{r gprinci1 ,fig.cap = "Durée de séjour selon la présence d'une sarcopénie"}
tt %>% 
  ggplot() + 
  aes(x = sarcopenie, y = duree.de.sejour.jour, fill = sarcopenie) +
  geom_violin() +
  geom_boxplot(width = 0.2, fill = "gray") +
      labs(title = "Durée de séjour selon la présence d'une sarcopénie",
         subtitle = "",
...
```{r p1reg}
ll <- lm(duree.de.sejour.jour~sarcopenie + age + urgence + stade.t, data = tt)
tbl_regression(ll)
```
...
Les données discrètes ont été décrites par leur fréquence exprimée en pourcentage avec son intervalle de confiance à 95 % et ont été comparées par le test exact de Fisher vu la faible effectif. Les intervalles de confiance n’ont été réalisés qu’après transformation angulaire. Les données numériques ont été décrites par leur moyenne (avec son intervalle de confiance à 95 % calculé par bootstrap) et l’écart-type. Les données continues ont été comparées par le test de Student après vérification de l’égalité des variances.
